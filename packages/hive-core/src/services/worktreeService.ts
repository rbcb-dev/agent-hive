...
export class WorktreeService {
  ...
  private async getStepStatusPath(
    feature: string,
    step: string,
  ): Promise<string> {
  ...
  async getDiff(
    ...
    try {
      ...
      if (hasStaged) {
        ...
        stat = diffContent
          ? await worktreeGit.diff(['--cached', '--stat'])
          : '';
      } else {
        diffContent = await worktreeGit.diff([`${base}..HEAD`]).catch(() => '');
        stat = diffContent
          ? await worktreeGit.diff([`${base}..HEAD`, '--stat'])
          : '';
      }
  ...
  async cleanup(
    feature?: string,
  ): Promise<{ removed: string[]; pruned: boolean }> {
    ...
    const features = feature
      ? [feature]
      : await fs.readdir(worktreesDir).catch(() => []);
  ...
  async checkConflictsFromSavedDiff(
    diffPath: string,
    reverse = false,
  ): Promise<string[]> {
    try {
  ...
  async commitChanges(
    ...
    try {
      ...
      const err = error as { message?: string };
      const currentSha = (
        await worktreeGit.revparse(['HEAD']).catch(() => '')
      ).trim();
      return {
        committed: false,
        sha: currentSha,
