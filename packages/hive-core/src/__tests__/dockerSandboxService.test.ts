...
describe('DockerSandboxService', () => {
  ...
  describe('buildRunCommand', () => {
    test('builds basic docker run command', () => {
      const result = DockerSandboxService.buildRunCommand(
        '/path/to/worktree',
        'npm test',
        'node:22-slim',
      );
      expect(result).toBe(
        "docker run --rm -v /path/to/worktree:/app -w /app node:22-slim sh -c 'npm test'",
      );
    });
    ...
    test('handles commands with single quotes', () => {
      ...
      expect(result).toBe(
        "docker run --rm -v /path/to/worktree:/app -w /app node:22-slim sh -c 'echo '\\''hello'\\'''",
      );
    });
    ...
    test('handles complex commands', () => {
      ...
      expect(result).toBe(
        "docker run --rm -v /path/to/worktree:/app -w /app node:22-slim sh -c 'bun test && echo done'",
      );
    });
  });
  ...
  describe('wrapCommand', () => {
    test('returns command unchanged when mode is none', async () => {
      await fs.writeFile(path.join(tempDir, 'package.json'), '{}');
      const result = DockerSandboxService.wrapCommand(tempDir, 'npm test', {
        mode: 'none',
      });
    ...
    test('wraps command when image is detected', async () => {
      ...
      expect(result).toBe(
        `docker run --rm -v ${tempDir}:/app -w /app node:22-slim sh -c 'npm test'`,
      );
    });
    ...
    test('uses explicit image override even when Dockerfile exists', async () => {
      ...
      expect(result).toBe(
        `docker run --rm -v ${tempDir}:/app -w /app node:20 sh -c 'npm test'`,
      );
    });

    test('strips HOST: prefix and returns unwrapped command', async () => {
      await fs.writeFile(path.join(tempDir, 'package.json'), '{}');
      const result = DockerSandboxService.wrapCommand(
        tempDir,
        'HOST: npm test',
        { mode: 'docker' },
      );
    ...
    test('handles HOST: prefix with mode none', () => {
      const result = DockerSandboxService.wrapCommand(tempDir, 'HOST: ls -la', {
        mode: 'none',
      });
  ...
  describe('containerName', () => {
    test('extracts feature and task from worktree path', () => {
      const worktreePath =
        '/home/user/project/.hive/.worktrees/my-feature/my-task';
    ...
    test('handles complex feature and task names', () => {
      const worktreePath =
        '/repo/.hive/.worktrees/v1.2.0-tighten-gates/07-implement-persistent-sandbox';
      const result = DockerSandboxService.containerName(worktreePath);
      expect(result).toBe(
        'hive-v1-2-0-tighten-gates-07-implement-persistent-sandbox',
      );
    });
    ...
    test('truncates names longer than 63 characters', () => {
      const worktreePath =
        '/repo/.hive/.worktrees/very-long-feature-name-that-goes-on-and-on/another-very-long-task-name';
      const result = DockerSandboxService.containerName(worktreePath);
  ...
  describe('ensureContainer', () => {
    test('returns existing container name when already running', () => {
      ...
      const worktreePath = '/repo/.hive/.worktrees/my-feature/my-task';
      const result = DockerSandboxService.ensureContainer(
        worktreePath,
        'node:22-slim',
      );
  ...
  describe('buildExecCommand', () => {
    test('produces correct docker exec command', () => {
      const result = DockerSandboxService.buildExecCommand(
        'hive-my-feature-my-task',
        'npm test',
      );
      expect(result).toBe(
        "docker exec hive-my-feature-my-task sh -c 'npm test'",
      );
    });
    ...
    test('escapes single quotes in commands', () => {
      ...
      expect(result).toBe(
        "docker exec my-container sh -c 'echo '\\''hello world'\\'''",
      );
    });
    ...
    test('handles complex commands with pipes and redirects', () => {
      ...
      expect(result).toBe(
        "docker exec my-container sh -c 'cat file.txt | grep test'",
      );
    });
  });
