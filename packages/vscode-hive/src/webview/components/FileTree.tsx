...
function getStatusColor(status: FileTreeItem['status']): string {
  switch (status) {
    case 'A': return 'var(--vscode-gitDecoration-addedResourceForeground, #81b88b)';
    case 'M': return 'var(--vscode-gitDecoration-modifiedResourceForeground, #e2c08d)';
    case 'D': return 'var(--vscode-gitDecoration-deletedResourceForeground, #c74e39)';
    case 'R': return 'var(--vscode-gitDecoration-renamedResourceForeground, #73c991)';
    default: return 'var(--vscode-foreground)';
  }
}

export function FileTree({ files, selectedFile, onSelectFile }: FileTreeProps): React.ReactElement {
  if (files.length === 0) {
    return (
