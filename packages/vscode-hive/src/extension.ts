...
class HiveExtension {
  ...
  registerCommands(): void {
    ...
    this.context.subscriptions.push(
      ...
      vscode.commands.registerCommand(
        'hive.openTask',
        (item: { featureName?: string; folder?: string }) => {
        ...
        async (item: { featureName?: string }) => {
          if (item?.featureName && this.workspaceRoot) {
            ...
            if (comments.length > 0) {
              vscode.window.showWarningMessage(
                `Hive: Cannot approve - ${comments.length} unresolved comment(s). Address them first.`,
              );
              return;
            }
      ...
      vscode.commands.registerCommand(
        'hive.startTask',
        async (item: { featureName?: string; folder?: string }) => {
      ...
      vscode.commands.registerCommand('hive.plan.doneReview', async () => {
        ...
        const userInput = await vscode.window.showInputBox({
          prompt: inputPrompt,
          placeHolder: hasComments
            ? 'Additional feedback (optional)'
            : 'e.g., "looks good" to approve, or describe changes needed',
        });
