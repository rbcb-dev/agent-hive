...
class PlanItem extends vscode.TreeItem {
  constructor(
    ...
    this.contextValue =
      featureStatus === 'planning' ? 'plan-draft' : 'plan-approved';
...
export class HiveSidebarProvider
  implements vscode.TreeDataProvider<SidebarItem>
{
  ...
  private getStatusGroups(): StatusGroupItem[] {
    ...
    for (const feature of features) {
      if (feature.feature.status === 'executing') {
        ...
        feature.feature.status === 'planning' ||
  ...
  private getAllFeatures(): FeatureItem[] {
    ...
    for (const name of dirs) {
      ...
      const feature: FeatureJson = JSON.parse(
  ...
  private getTasks(
    featureName: string,
    tasks: Array<{ folder: string; status: TaskStatus }>,
  ): TaskItem[] {
    const featurePath = path.join(
      this.workspaceRoot,
      '.hive',
  ...
  private getTaskList(
    featureName: string,
  ): Array<{ folder: string; status: TaskStatus }> {
