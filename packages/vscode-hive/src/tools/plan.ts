  ...
  return [
    {
      ...
      modelDescription:
        'Write or update the plan.md for a feature. The plan defines tasks to execute. Use markdown with ### numbered headers for tasks. Clears existing comments when plan is rewritten.',
      inputSchema: {
        type: 'object',
        properties: {
          feature: {
          ...
          content: {
            type: 'string',
            description:
              'Plan content in markdown. Use ### 1. Task Name format for tasks.',
          },
        },
        required: ['feature', 'content'],
      },
      invoke: async (input) => {
        const { feature, content } = input as {
          feature: string;
          content: string;
        };
    ...
    },
    {
      ...
      modelDescription:
        'Read the plan.md and any user comments for a feature. Use to check plan content, status, and user feedback before making changes.',
      readOnly: true,
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      modelDescription:
        'Approve a plan for execution. Use after user has reviewed the plan and resolved any comments. Changes feature status to approved.',
      inputSchema: {
        type: 'object',
        properties: {
          feature: {
