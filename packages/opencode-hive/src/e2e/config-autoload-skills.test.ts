...
describe('config hook autoLoadSkills injection', () => {
  ...
  it('injects default autoLoadSkills into agent prompt in config hook (unified mode)', async () => {
    ...
    const scoutPrompt = opencodeConfig.agent['scout-researcher']
      ?.prompt as string;
    ...
    const foragerPrompt = opencodeConfig.agent['forager-worker']
      ?.prompt as string;
    expect(foragerPrompt).toBeDefined();
    const tddSkill = BUILTIN_SKILLS.find(
      (s) => s.name === 'test-driven-development',
    );
    const verificationSkill = BUILTIN_SKILLS.find(
      (s) => s.name === 'verification-before-completion',
    );
  ...
  it('injects autoLoadSkills for all agents in dedicated mode', async () => {
    ...
    const architectPrompt = opencodeConfig.agent['architect-planner']
      ?.prompt as string;
    ...
    const swarmPrompt = opencodeConfig.agent['swarm-orchestrator']
      ?.prompt as string;
  ...
  it('system.transform does NOT inject skills (legacy path removed)', async () => {
    ...
    expect(joinedWithoutAgent).not.toContain(
      parallelExplorationSkill!.template,
    );

    // HIVE_SYSTEM_PROMPT should still be there
    expect(joinedWithoutAgent).toContain(
...
describe('file-based skill fallback in autoLoadSkills', () => {
  ...
  it('builtin skill wins over file-based skill with same ID', async () => {
    ...
    const builtinBrainstorming = BUILTIN_SKILLS.find(
      (s) => s.name === 'brainstorming',
    );
  ...
  it('disableSkills prevents injection of a file-based skill', async () => {
    ...
    expect(hiveMasterPrompt).not.toContain(
      "This should NOT appear because it's disabled.",
    );
