...
async function getDefaultModel(
  ...
  const providerEntry = providers.find(
    (p) => isRecord(p) && isRecord(p.models),
  );
  if (!isRecord(providerEntry)) return null;

  const providerID =
    typeof providerEntry.id === 'string' ? providerEntry.id : null;
  ...
  const defaultModelID =
    typeof defaultMap[providerID] === 'string'
...
describe('e2e: OpenCode runtime loads opencode-hive', () => {
  it('exposes hive tools via /experimental/tool/ids', async () => {
    ...
    fs.mkdirSync(path.join(projectDir, '.opencode', 'plugin'), {
      recursive: true,
    });
    ...
    try {
      ...
      console.warn(
        '[hive] Skipping runtime e2e test: unable to bind localhost port',
        err,
      );
      return;
    }
    ...
    try {
      ...
      console.warn(
        '[hive] Skipping runtime e2e test: unable to start opencode server',
        err,
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          '[hive] Skipping runtime e2e test: prompt did not complete',
          err,
        );
